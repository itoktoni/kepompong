  ...
  <div class="flex items-center gap-2">
    ...
    <div class="relative">
      <button class="w-9 h-9 flex items-center justify-center rounded-full bg-white border-2 border-[#B7D9BC] shadow-sm text-primary hover:opacity-80 transition-opacity duration-200 relative"
        ...
        {#if $unreadCount > 0}
          <span style="letter-spacing: 2px;" class="absolute -top-2 -right-2 min-w-[19px] h-[22px] flex items-center justify-center px-1.5 bg-error text-white text-[9px] rounded-full border-2 border-canvas-cream">{$unreadCount > 99 ? '99+' : $unreadCount}</span>
        {/if}
      </button>
