...
<div style="width:{PAGE_W}px;height:{PAGE_H}px;box-sizing:border-box;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:28px;background:linear-gradient(135deg,#E0F2F1 0%,#fff 100%);font-family:'Fredoka',sans-serif">
  {#if cover}
    <img src={cover} alt="" style="width:100%;height:auto;object-fit:contain;border-radius:14px;border:3px solid #80CBC4;box-shadow:0 4px 16px rgba(0,0,0,0.12);margin-bottom:18px" />
  {:else}
    <div style="font-size:48px;margin-bottom:14px">&#x1F9E9;</div>
  {/if}
  <h1 style="font-size:22px;color:#00695C;font-weight:800;text-align:center;margin:0 0 12px;line-height:1.2">{item.title}</h1>
...
{#each questions as q, i}
  <div style="width:{PAGE_W}px;height:{PAGE_H}px;box-sizing:border-box;display:flex;flex-direction:column;padding:28px 28px 24px;font-family:'Fredoka',sans-serif;background:#fff">
    <div style="text-align:center;margin-bottom:12px">
      <span style="display:inline-block;width:24px;height:24px;border-radius:50%;background:#00695C;color:#fff;font-size:10px;font-weight:700;line-height:24px">{i + 1}</span>
    </div>
      ...
      {#if q.options?.length}
        <div style="display:flex;flex-direction:column;gap:6px;max-width:260px;width:100%">
          {#each q.options as opt, j}
            <div style="padding:7px 10px;background:#E0F2F1;border-radius:6px;font-size:10px;color:#1F2937;text-align:center">{opt}</div>
          {/each}
        </div>
      {/if}
      {#if q.answer}
        <p style="font-size:11px;color:#00695C;font-weight:700;margin:12px 0 0;word-break:normal;overflow-wrap:normal;hyphens:none">&#x2705; {q.answer}</p>
      {/if}
    </div>
