      ...
      <div style="display:flex;gap:10px;align-items:flex-start">
        ...
        <div style="flex:1">
          ...
          {#if ex.options?.length}
            <ul style="margin:4px 0 0;padding-left:20px">{#each ex.options as opt}<li style="margin-bottom:2px;font-size:13px;line-height:1.5">{opt}</li>{/each}</ul>
          {/if}
          {#if ex.answer}
            <div style="margin-top:4px;font-weight:700;color:#176C33;font-size:13px">&#x2705; {ex.answer}</div>
          {/if}
        </div>
      </div>
    </div>
