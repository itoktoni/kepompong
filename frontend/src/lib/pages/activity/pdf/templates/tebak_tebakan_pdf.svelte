...
<div style="width:794px;height:{PAGE_H}px;box-sizing:border-box;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:48px;background:linear-gradient(135deg,#FFF8E1 0%,#fff 100%);font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif">
  {#if cover}
    <img src={cover} alt="" style="width:320px;height:320px;object-fit:cover;border-radius:24px;border:4px solid #FFE082;box-shadow:0 8px 32px rgba(0,0,0,0.12);margin-bottom:32px" />
  {:else}
...
{#each questions as q, i}
  <div style="width:794px;height:{PAGE_H}px;box-sizing:border-box;display:flex;flex-direction:column;padding:48px 48px 40px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#fff">
    <div style="text-align:center;margin-bottom:20px">
      <span style="display:inline-block;width:36px;height:36px;border-radius:50%;background:#F57F17;color:#fff;font-size:16px;font-weight:700;line-height:36px">{i + 1}</span>
    </div>
      ...
      {#if q.options?.length}
        <div style="display:flex;flex-direction:column;gap:10px;max-width:450px;width:100%">
          {#each q.options as opt}
            <div style="padding:12px 16px;background:#FFF8E1;border-radius:10px;font-size:17px;color:#1F2937;text-align:center">{opt}</div>
          {/each}
        </div>
      {/if}
      {#if q.answer}
        <p style="font-size:18px;color:#F57F17;font-weight:700;margin:20px 0 0">&#x2705; {q.answer}</p>
      {/if}
      {#if q.explanation}
        <p style="font-size:15px;color:#6B7280;margin:12px 0 0;max-width:500px;text-align:center">{q.explanation}</p>
      {/if}
    </div>
