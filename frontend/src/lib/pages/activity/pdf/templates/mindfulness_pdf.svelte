<script>
  ...
  const benefit = item.benefit || item.data?.benefit || ''
  const allSteps = pages.length ? pages : steps
  const cover = coverImage(type, item)
</script>

<div style="width:{PAGE_W}px;height:{PAGE_H}px;box-sizing:border-box;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:28px;background:linear-gradient(135deg,#E8F5E9 0%,#fff 100%);font-family:'Fredoka',sans-serif">
  {#if cover}
    <img src={cover} alt="" style="width:100%;height:auto;object-fit:contain;border-radius:14px;border:3px solid #A5D6A7;box-shadow:0 4px 16px rgba(0,0,0,0.12);margin-bottom:18px" />
  {:else}
    <div style="font-size:48px;margin-bottom:14px">&#x1F9D8;</div>
  {/if}
  <h1 style="font-size:22px;color:#2E7D32;font-weight:800;text-align:center;margin:0 0 12px;line-height:1.2">{item.title}</h1>
...
{#each allSteps as step, i}
  ...
  <div style="width:{PAGE_W}px;height:{PAGE_H}px;box-sizing:border-box;display:flex;flex-direction:column;padding:28px 28px 24px;font-family:'Fredoka',sans-serif;background:#fff">
    <div style="text-align:center;margin-bottom:12px">
      <span style="display:inline-block;width:24px;height:24px;border-radius:50%;background:#2E7D32;color:#fff;font-size:10px;font-weight:700;line-height:24px">{i + 1}</span>
    </div>
...
{#if benefit}
  <div style="width:{PAGE_W}px;height:{PAGE_H}px;box-sizing:border-box;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:36px;background:linear-gradient(135deg,#E8F5E9 0%,#fff 100%);font-family:'Fredoka',sans-serif">
    <div style="font-size:36px;margin-bottom:12px">&#x1F31F;</div>
    <h2 style="font-size:16px;color:#2E7D32;font-weight:800;margin:0 0 14px">Manfaat</h2>
    <p style="font-size:12px;color:#1F2937;text-align:center;line-height:1.7;max-width:320px;font-weight:500;margin:0;word-break:normal;overflow-wrap:normal;hyphens:none">{benefit}</p>
    <div style="margin-top:auto;padding-top:20px;font-size:8px;color:#9CA3AF">{APP_NAME}</div>
  </div>
{/if}
