  ...
  <div class="p-5 flex flex-col flex-1">
    <div class="flex items-start justify-between mb-3">
      ...
      {#if item.questions?.length}
        <span class="text-xs font-bold px-2 py-1 rounded-full" style="background: {bg}; color: #673AB7">
    ...
    {#if item.questions?.length}
      <div class="bg-white rounded-xl p-3 mb-3 border border-[#B7D9BC]/50">
        <p class="text-xs text-on-surface-variant">
          <span class="font-bold text-primary">Contoh:</span> {item.questions[0].q || item.questions[0]}
        </p>
      </div>
    {/if}
    {#if item.views}
