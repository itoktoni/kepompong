  ...
  <div class="p-5 flex flex-col flex-1">
    <div class="flex items-start justify-between mb-3">
      ...
      {#if item.exercises?.length}
        <span class="text-xs font-bold px-2 py-1 rounded-full" style="background: {bg}; color: #283593">
          {item.exercises.length} latihan
        </span>
      {/if}
    </div>
    ...
    {#if item.exercises?.length}
      <div class="bg-white rounded-xl p-3 mb-3 border border-[#B7D9BC]/50">
        <p class="text-xs text-on-surface-variant">
          <span class="font-bold text-primary">Contoh:</span> {item.exercises[0].title || item.exercises[0]}
        </p>
      </div>
    {/if}
    {#if item.moral}
