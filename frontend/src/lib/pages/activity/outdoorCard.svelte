<script>
  ...
  function prevPage() {
    if (isFinished) { isFinished = false; return }
    ...
    <div class="bg-white rounded-[24px] overflow-hidden shadow-lg border-4 relative"
      ...
      <div class="aspect-square p-2 overflow-hidden relative rounded-t-[20px]">
        ...
        <div class="absolute bottom-2 right-2">
          {#if totalPages > 0}
            <div class="bg-white/90 backdrop-blur-sm rounded-full mr-1 mb-1 px-2.5 py-1 text-[10px] font-bold shadow-sm" style="color: #009688">
              {totalPages} langkah
            </div>
          {/if}
        </div>
      ...
      <div class="px-3 py-2.5 flex items-center justify-between bg-success-soft">
        ...
        <div class="flex items-center gap-2">
          <span onclick={(e) => { e.stopPropagation(); handleDownload() }}
            class="font-medium border-[#B7D9BC] flex items-center justify-center text-xs hover:bg-success-soft transition-colors cursor-pointer shrink-0 {downloading ? 'opacity-50 pointer-events-none' : ''}"
          ...
          <div class="h-full overflow-y-auto"
            ...
            <div class="flex flex-col">
              ...
              <div class="px-4 py-4 space-y-3">
                ...
                {#if currentPage.observation}
                  <div class="bg-success-soft rounded-2xl p-4 border-2 border-[#B7D9BC] shadow-sm">
                    <p class="text-xs font-bold text-primary mb-1">🔍 Observasi</p>
                    <p class="text-sm text-on-surface leading-relaxed">{currentPage.observation}</p>
                  </div>
                {/if}
              </div>
            </div>
