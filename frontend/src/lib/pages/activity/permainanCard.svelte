<script>
  ...
  const rules = $derived(item.rules || [])
  const materials = $derived(item.materials || item.data?.materials || [])
  const how = $derived(item.how || item.data?.how || '')
  const players = $derived(item.players || item.data?.players || '')
  const pages = $derived(rules.length > 0 ? rules.map((r, i) => ({ num: i + 1, text: typeof r === 'string' ? r : r.text || r })) : [])
  ...
  function getRuleText(page) {
    if (typeof page === 'string') return page
    return page.text || page.rule || ''
  }
</script>
    ...
    <div class="bg-white rounded-[24px] overflow-hidden shadow-lg border-4 relative"
      ...
      <div class="aspect-square p-2 overflow-hidden relative rounded-t-[20px]">
        ...
        <div class="absolute bottom-2 right-2">
          {#if totalPages > 0}
            <div class="bg-white/90 backdrop-blur-sm rounded-full mr-1 mb-1 px-2.5 py-1 text-[10px] font-bold shadow-sm" style="color: #E91E63">
              {totalPages} aturan
            </div>
          {/if}
        </div>
      ...
      <div class="px-3 py-2.5 flex items-center justify-between bg-success-soft">
        ...
        <div class="flex items-center gap-2">
          <span onclick={(e) => { e.stopPropagation(); handleDownload() }}
            class="font-medium flex items-center justify-center text-xs hover:bg-success-soft transition-colors cursor-pointer shrink-0 {downloading ? 'opacity-50 pointer-events-none' : ''}"
            ...
            <div class="flex flex-col">
              ...
              <div class="px-4 py-4 space-y-3">
                {#if currentPageIndex === 0}
                  {#if players}
                    <div class="bg-[#FCE4EC] rounded-2xl p-4 border-2 border-[#F48FB1] shadow-sm">
                      <p class="text-xs font-bold text-[#C2185B] mb-1">👥 Pemain</p>
                      <p class="text-sm text-on-surface leading-relaxed">{players}</p>
                    </div>
                  {/if}
                  {#if how}
                    <div class="bg-[#FCE4EC] rounded-2xl p-4 border-2 border-[#F48FB1] shadow-sm">
                      <p class="text-xs font-bold text-[#C2185B] mb-1">📋 Cara Bermain</p>
                      <p class="text-sm text-on-surface leading-relaxed">{how}</p>
                    </div>
                  {/if}
                  {#if materials.length > 0}
                    <div class="bg-[#FCE4EC] rounded-2xl p-4 border-2 border-[#F48FB1] shadow-sm">
                      <p class="text-xs font-bold text-[#C2185B] mb-2">🧰 Bahan yang dibutuhkan</p>
                      <ul class="space-y-1">
                        {#each materials as mat}
                          <li class="text-sm text-on-surface flex items-start gap-2">
                            <span class="text-[#C2185B] shrink-0">•</span>
                            <span>{mat}</span>
                          </li>
                        {/each}
                      </ul>
                ...
                <div class="bg-white rounded-2xl p-4 border-2 border-[#B7D9BC] shadow-sm">
                  <p class="font-body-md text-body-md text-on-surface leading-relaxed">{getRuleText(currentPage)}</p>
                </div>
              </div>
            </div>
          </div>
