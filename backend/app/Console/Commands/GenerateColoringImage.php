...
class GenerateColoringImage extends Command
{
    protected $signature = 'generate:coloring-image
    ...
    protected $description = 'Generate coloring page images using AI and split into pages';
    ...
    public function handle(ImageGeneratorService $generator): int
    {
        ...
        $activities = Activity::where('type', 'coloring')
        ...
        if ($activities->isEmpty()) {
            $this->info('No pending coloring activities with prompts found.');
            return self::SUCCESS;
        }

        $this->info("Found {$activities->count()} pending coloring activities.");
        $this->newLine();
    ...
    private function processActivity(ImageGeneratorService $generator, Activity $activity): int
    {
        ...
        $pagesCount = $this->option('pages')
            ?: (isset($activity->data['items']) ? count($activity->data['items']) + 1 : 12);
        ...
        $folder = "images/coloring/{$activity->slug}";
